...
public class UICenterOnChild : MonoBehaviour
{
	...
	void CenterOn (Transform target, Vector3 panelCenter)
	{
		if (target != null && mScrollView != null && mScrollView.panel != null)
		{
			...
			//lockes the feed when cetring on top element
			//only works for downward vertical feed
			Vector3 newPos;

			if(theFeed != null){
				if(-cp.y < theFeed.basePos.y){
					// Spring the panel to base position
					newPos = theFeed.basePos;
				}else{
					newPos = panelTrans.localPosition - localOffset;
				}
			}else{
				newPos = panelTrans.localPosition - localOffset;
			}
			...
			SpringPanel.Begin(mScrollView.panel.cachedGameObject, newPos, springStrength).onFinished = onFinished;
		}
